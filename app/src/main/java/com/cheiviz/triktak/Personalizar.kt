...
class Personalizar : BaseActivity() {
    ...
    private lateinit var switchPrivado: Switch
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        switchGlobal = findViewById(R.id.switchChatGlobal)
        switchPrivado = findViewById(R.id.switchChatPrivado)



        // Cargar valores actuales
        lifecycleScope.launch {
            switchGlobal.isChecked = prefs.leerNotifGlobal()
            switchPrivado.isChecked = prefs.leerNotifPrivado()
        }

        // Guardar cambios
        switchGlobal.setOnCheckedChangeListener { _, isChecked ->
            lifecycleScope.launch {
                prefs.setNotifGlobal(isChecked)
            }
        }

        switchPrivado.setOnCheckedChangeListener { _, isChecked ->
            lifecycleScope.launch {
                prefs.setNotifPrivado(isChecked)
            }
        }
